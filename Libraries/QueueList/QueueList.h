...
public:
    ...
    void setPrinter (Print & p) volatile;

private:
    // exit report method in case of error.
    void exit (const char * m) const volatile;

    // led blinking method in case of error.
    void blink () const volatile;
...
// push an item to the queue.
template<typename T>
void QueueList<T>::push (const T i) volatile{
...
// get an item from the queue.
template<typename T>
T QueueList<T>::peek () const volatile{
    // check if the queue is empty.
    if (isEmpty ())
...
// check if the queue is empty.
template<typename T>
bool QueueList<T>::isEmpty () const volatile{
    return head == NULL;
}

// get the number of items in the queue.
template<typename T>
int QueueList<T>::count () const volatile{
    return size;
}

// set the printer of the queue.
template<typename T>
void QueueList<T>::setPrinter (Print & p) volatile{
    printer = &p;
}

// exit report method in case of error.
template<typename T>
void QueueList<T>::exit (const char * m) const volatile{
...
// led blinking method in case of error.
template<typename T>
void QueueList<T>::blink () const volatile{
