...
*/
void FreeIMU::getEulerRad(float * angles, float * q) {
...
*/
void FreeIMU::getEuler(float * angles, float * q) {
  getEulerRad(angles, q);
 ...
 * @param ypr three floats array which will be populated by Yaw, Pitch and Roll angles in radians
*/
void FreeIMU::getYawPitchRoll180(float * ypr, float * q, float * val) {
	// float q[4];				// quaternion
	// float val[11];
	float gx, gy, gz;		// estimated gravity direction
	...
	//calculating yaw
	//ypr[0] = atan2(2 * q[1] * q[2] - 2 * q[0] * q[3], 2 * q[0]*q[0] + 2 * q[1] * q[1] - 1);	
	ypr[0] = val[9]; // yaw = heading
	if (ypr[0] > 180.) ypr[0] -= 360.;
	ypr[0] *= (M_PI / 180.); // deg to radians
	
	//calculating Pitch1
	//Serial.print(gx); Serial.print("       "); Serial.print(gz); Serial.print("       ");
	if(gx > 0 && gz <= 0) {
		ypr[1] = M_PI - atan(gx / sqrt(gy*gy + gz*gz));
	} else if(gx < 0 && gz < 0) {
		ypr[1] = (-M_PI - atan(gx / sqrt(gy*gy + gz*gz)));
	} else  {
		ypr[1] = atan(gx / sqrt(gy*gy + gz*gz));
