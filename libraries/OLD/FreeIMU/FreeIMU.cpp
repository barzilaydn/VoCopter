...
float FreeIMU::getBaroAlt(float sea_press) {
    ...
    float temp = baro.getTemperatureC();
    ...
    float alt = ((pow((sea_press / press), 1/5.257) - 1.0) * (temp + 273.15)) / 0.0065;
    return baro.getAltitude(press, (sea_press * 100));//(baro.getAltitude(press, (sea_press * 100)) + alt) / 2;
  #endif
}
...
/**
 * Returns an altitude estimate from barometer fused with accelerometer readings
*/
float FreeIMU::getEstimatedAlt() {
  static uint8_t inited = 0;
  static int16_t AltErrorI = 0;
  static float AccScale  = 9.80665f;
  ...
  InstAcc = (ypr[0] * (1 - 200 * invSqrt(isq(ypr[2]) + isq(ypr[1]) + isq(ypr[0])))) * AccScale + (Ki) * AltErrorI;
  Serial.println(InstAcc);
  //Integrators
...
void FreeIMU::q_mult(float * base, float * q1, float * q2)
{
  base[0] = q1[3] * q2[0] + q1[0] * q2[3] + q1[1] * q2[2] - q1[2] * q2[1];
  base[1] = q1[3] * q2[1] + q1[1] * q2[3] + q1[2] * q2[0] - q1[0] * q2[2];
  base[2] = q1[3] * q2[2] + q1[2] * q2[3] + q1[0] * q2[1] - q1[1] * q2[0];
  base[3] = q1[3] * q2[3] - q1[0] * q2[0] - q1[1] * q2[1] - q1[2] * q2[2];
}
...
*/
void FreeIMU::gravityCompensateAcc(float * base, float * acc, float * q) {
  float g[3];
  ...
  base[0] = acc[0] / 1000 - g[0];
  base[1] = acc[1] / 1000 - g[1];
  base[2] = acc[2] / 1000 - g[2];
  base[3] = 0.0f;
}
